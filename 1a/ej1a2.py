...
def create_event(name: str, datetime_start: datetime, timezone_str: str) -> Dict[str, str]:
    ...
    return {"name" : name, "datetime_start" : pytz.timezone(timezone_str).localize(datetime_start), "timezone" : timezone_str}


def time_until_event(event: Dict[str, str]) -> timedelta:
    # Write here your code
    return event["datetime_start"] - pytz.timezone(event["timezone"]).localize(datetime.now())
...
def find_next_event(events: List[Dict[str, str]]) -> Optional[Dict[str, str]]:
    # Write here your code
    next_event = create_event("", datetime(1987, 1, 1, 0, 0), "UTC")
    for event in events:
        if time_until_event(event).total_seconds() > 0:
            if time_until_event(event).total_seconds() < time_until_event(next_event).total_seconds() or time_until_event(next_event).total_seconds() < 0:
                next_event = event
            print(time_until_event(event).total_seconds())
    
    if time_until_event(next_event).total_seconds() > 0:
        return next_event
